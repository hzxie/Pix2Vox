...
class Refiner(torch.nn.Module):
    ...
    def forward(self, coarse_voxels):
        ...
        # print(voxels_32_l.size())       # torch.Size([24, 1, 32, 32, 32])
        voxels_16_l      = self.layer1(voxels_32_l)
        # print(voxels_16_l.size())       # torch.Size([24, 32, 16, 16, 16])
        voxels_8_l       = self.layer2(voxels_16_l)
        # print(voxels_8_l.size())        # torch.Size([24, 64, 8, 8, 8])
        voxels_4_l       = self.layer3(voxels_8_l)
        # print(voxels_4_l.size())        # torch.Size([24, 128, 4, 4, 4])
        flatten_features = self.layer4(voxels_4_l.view(-1, 8192))
        # print(flatten_features.size())  # torch.Size([24, 2048])
        flatten_features = self.layer5(flatten_features)
        # print(flatten_features.size())  # torch.Size([24, 8192])
        voxels_4_r       = voxels_4_l + flatten_features.view(-1, self.cfg.CONST.N_VOX * 4, 4, 4, 4)
        # print(voxels_4_r.size())        # torch.Size([24, 128, 4, 4, 4])
        voxels_8_r       = voxels_8_l + self.layer6(voxels_4_r)
        # print(voxels_8_r.size())        # torch.Size([24, 64, 8, 8, 8])
        voxels_16_r      = voxels_16_l + self.layer7(voxels_8_r)
        # print(voxels_16_r.size())       # torch.Size([24, 32, 16, 16, 16])
        voxels_32_r      = (voxels_32_l + self.layer8(voxels_16_r)) * 0.5
        # print(voxels_32_r.size())       # torch.Size([24, 1, 32, 32, 32])
