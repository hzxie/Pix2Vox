...
class Refiner(torch.nn.Module):
    def __init__(self, cfg):
        ...
        self.layer3 = torch.nn.Sequential(
            torch.nn.Conv3d(5, 5, kernel_size=3, dilation=2, bias=cfg.NETWORK.TCONV_USE_BIAS, padding=2),
            torch.nn.BatchNorm3d(5),
            torch.nn.LeakyReLU(cfg.NETWORK.LEAKY_VALUE)
        )
        self.layer4 = torch.nn.Sequential(
            torch.nn.Conv3d(5, 5, kernel_size=3, dilation=4, bias=cfg.NETWORK.TCONV_USE_BIAS, padding=4),
            torch.nn.BatchNorm3d(5),
            torch.nn.LeakyReLU(cfg.NETWORK.LEAKY_VALUE)
        )
        self.layer5 = torch.nn.Sequential(
            torch.nn.Conv3d(5, 5, kernel_size=3, dilation=8, bias=cfg.NETWORK.TCONV_USE_BIAS, padding=8),
            torch.nn.BatchNorm3d(5),
            torch.nn.LeakyReLU(cfg.NETWORK.LEAKY_VALUE)
        )
        self.layer6 = torch.nn.Sequential(
            torch.nn.Conv3d(5, 5, kernel_size=3, dilation=16, bias=cfg.NETWORK.TCONV_USE_BIAS, padding=16),
            torch.nn.BatchNorm3d(5),
            torch.nn.LeakyReLU(cfg.NETWORK.LEAKY_VALUE)
        )
        ...
        self.layer8 = torch.nn.Sequential(
            torch.nn.Conv3d(5, 1, kernel_size=3, dilation=1, bias=cfg.NETWORK.TCONV_USE_BIAS, padding=1),
            torch.nn.Sigmoid()
        )

    def forward(self, gen_voxels, raw_features):
        raw_features    = self.layer1(raw_features)
        # print(raw_features.size())    # torch.Size([batch_size, 512, 16, 16])
        raw_features    = raw_features.view((-1, 4, self.cfg.CONST.N_VOX, self.cfg.CONST.N_VOX, self.cfg.CONST.N_VOX))
        # print(raw_features.size())    # torch.Size([batch_size, 4, 32, 32, 32])
        ...
        # print(voxel_features.size())  # torch.Size([batch_size, 5, 32, 32, 32])
