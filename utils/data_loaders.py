...
class ModelNet40Dataset(torch.utils.data.dataset.Dataset):
    """ShapeNetDataset class used for PyTorch DataLoader"""
    def __init__(self, file_list_with_metadata, n_rendering_views, transforms=None):
        self.file_list = file_list_with_metadata
        self.transforms = transforms
    ...
    def __getitem__(self, idx):
        taxonomy_id, sample_name, rendering_images, voxel = self.get_datum(idx)

        if self.transforms:
            rendering_images, voxel = self.transforms(rendering_images, voxel)
        
        return taxonomy_id, sample_name, rendering_images, voxel
    ...
    def get_datum(self, idx):
        ...
        return taxonomy_id, sample_name, np.asarray(rendering_images), voxel

# //////////////////////////////// = End of ModelNet40Dataset Class Definition = ///////////////////////////////// #
...
class ModelNet40DataLoader:
    ...
    def get_dataset(self, dataset_type, total_views, n_rendering_views, transforms=None):
        ...
        for taxonomy in self.dataset_taxonomy:
            ...
            if dataset_type == DatasetType.TRAIN:
                samples = taxonomy['train']
                dataset_type_folder = 'train'
            elif dataset_type == DatasetType.TEST:
                samples = taxonomy['test']
                dataset_type_folder = 'test'
            elif dataset_type == DatasetType.VAL:
                samples = taxonomy['test']
                dataset_type_folder = 'test'
            
            files.extend(self.get_files_of_taxonomy(taxonomy_id, taxonomy_name, dataset_type_folder, samples, total_views))

        print('[INFO] %s Complete collecting files of the dataset. Total files: %d.' % (dt.now(), len(files)))
        return ModelNet40Dataset(files, n_rendering_views, transforms)

    def get_files_of_taxonomy(self, taxonomy_id, taxonomy_name, dataset_type_folder, samples, total_views):
        files_of_taxonomy = []
        n_samples = len(samples)

        for sample_idx, sample_name in enumerate(samples):
            # Get file path of voxels
            voxel_file_path = self.voxel_path_template % (taxonomy_name, dataset_type_folder, sample_name)
            if not os.path.exists(voxel_file_path):
                print('[WARN] %s Ignore sample %s/%s since voxel file not exists.' % (dt.now(), taxonomy_name, sample_name))
                continue

            # Get file list of rendering images
            rendering_image_indexes = range(1, total_views + 1)
            rendering_images_file_path = []
            for image_idx in rendering_image_indexes:
                img_file_path = self.rendering_image_path_template % (taxonomy_name, dataset_type_folder, sample_name, image_idx)
                # if not os.path.exists(img_file_path):
                #     print('[WARN] %s Ignore rendering image of sample %s/%s/%d since file not exists.' % \
                #         (dt.now(), taxonomy_name, sample_name, image_idx))
                #     continue
                rendering_images_file_path.append(img_file_path)
